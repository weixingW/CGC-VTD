...
"""class Emu3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # ==== BEGIN XFORMERS IMPLEMENTATION ====
        # Import xformers only when needed
        try:
            ...
            has_xformers = False
            warnings.warn(
                "xformers not available, falling back to standard attention implementation. "
                "Install xformers for faster and memory-efficient attention."
            )
        
        if has_xformers:
            # Reshape for xformers: [batch, heads, seq_len, dim] -> [batch, seq_len, heads, dim]
            ...
            if attention_mask is not None:
                # Fix for alignment issue: ensure proper memory alignment by padding to multiple of 8
                q_len = query_states.size(1)
                kv_seq_len = key_states.size(1)
                
                # Calculate padded dimensions (multiples of 8)
                padded_q_len = ((q_len + 7) // 8) * 8
                padded_kv_len = ((kv_seq_len + 7) // 8) * 8
                
                # Create properly aligned tensor and then slice it
                padded_mask = torch.zeros(
                    (bsz, self.num_heads, padded_q_len, padded_kv_len),
                    device=attention_mask.device,
                    dtype=query_states.dtype
                )

                # Copy values from the original mask
                if attention_mask.shape[1] == 1:
                    # If single-headed mask, expand to multi-headed
                    temp_mask = attention_mask.expand(bsz, 1, q_len, kv_seq_len)
                    # Convert values: 0 -> -inf, non-zero -> 0
                    temp_mask = temp_mask.masked_fill(temp_mask == 0, float("-inf"))
                    # Copy to each head in the padded mask
                    for h in range(self.num_heads):
                        padded_mask[:, h, :q_len, :kv_seq_len] = temp_mask[:, 0]
                else:
                    # If already multi-headed
                    temp_mask = attention_mask.to(query_states.dtype)
                    temp_mask = temp_mask.masked_fill(temp_mask == 0, float("-inf"))
                    padded_mask[:, :, :q_len, :kv_seq_len] = temp_mask
            
                # Use the properly aligned mask
                attn_bias = padded_mask[:, :, :q_len, :kv_seq_len]
            ...
            # Reshape the output: [batch, seq_len, heads, dim] -> [batch, seq_len, hidden_size]
            attn_output = attn_output.reshape(bsz, q_len, self.hidden_size)
        else:
            # Fall back to standard attention implementation
            
            query_states_dtype = query_states.dtype
            query_states_device = query_states.device
            ...
            #attn_weight_cpu = attn_weights.to("cpu")
            attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states_dtype)
            attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
            attn_weights = attn_weights.to(query_states_device)
            ...
            attn_output = attn_output.reshape(bsz, q_len, self.hidden_size)
        # ==== END XFORMERS IMPLEMENTATION ====
        
        # Get attention weights if needed for output
                    ...
                    if idx<AGG_LAYER:
                        ...
                        if vad==False:
                            if idx!=0:
                                ...
                                gen_attention_mask = _prepare_4d_causal_attention_mask(
...
class Emu3ForCausalLM(Emu3PreTrainedModel):
    ...
    def prepare_inputs_for_generation_cd(
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                if past_key_values.get_seq_length() == 0:
                    input_ids = kwargs.get("images_cd")
                cache_length = past_key_values.get_seq_length()
                past_length = past_key_values.seen_tokens
                max_cache_length = past_key_values.get_max_length()
            else:
        ...
        #    model_inputs = {"input_ids": kwargs.get("images_cd", None)}
