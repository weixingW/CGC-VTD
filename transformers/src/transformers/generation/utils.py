...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    cd_probs: Optional[torch.FloatTensor] = None
    cd_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        elif generation_mode == GenerationMode.VCD_GENERATION:
            if not model_kwargs["use_cache"]:
                raise ValueError("VCD generation requires `use_cache=True`")
            if self._is_stateful:
                raise ValueError(
                    f"VCD generation is not supported with stateful models, such as {self.__class__.__name__}"
                )
    ...
    def opera_beam_search(
        ...
        num_attn_candidates: Optional[int] = 3, 
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # if model is using input_embeds, then we need to also keep track the input_embeds
            if "inputs_embeds" in model_kwargs.keys():
                current_state["inputs_embeds"] = model_kwargs["inputs_embeds"].clone()
            #
                  
            # prepare model inputs 
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            # crop the past_key_values to match the current length
            #if len(model_kwargs["past_key_values"]) > 0:
            #    model_kwargs["past_key_values"].crop(cur_len-1)
            self.apply_hook_flag = True
            # forward pass to get next token
            outputs = self(
                **model_inputs,
                return_dict=True,
            ...
            if not "past_key_values" in model_kwargs.keys():
                ...
                if beam_idx is None:  # First step with past_key_values
                    attn_previous = outputs.attentions[-1].clone()
                else:  # Subsequent steps
                    assert attn_previous is not None
                    attn_previous = torch.cat([attn_previous, torch.zeros_like(attn_previous).sum(-1, keepdim=True)], -1)
                    curr_attn = copy.deepcopy(outputs.attentions[-1])
                    if curr_attn.size(-1) != attn_previous.size(-1):
                        print('breaking!')
                        curr_attn = F.pad(curr_attn, (0, attn_previous.size(-1) - curr_attn.size(-1)), mode='constant', value=0)
                        print('Padding output attention to match previous attention')
                        break
                        
                    attn_previous = torch.cat(
                        [attn_previous[beam_idx], curr_attn.max(1, keepdim=True).values.data], -2)
            ...
            next_token_scores = nn.functional.log_softmax(
                next_token_logits, dim=-1, dtype=next_token_logits.dtype
            )  # (batch_size * num_beams, vocab_size)

            next_token_scores_processed = logits_processor(input_ids, next_token_scores).to(next_token_scores.dtype)
            next_token_scores = next_token_scores_processed + beam_scores[:, None].expand_as(next_token_scores)
            next_token_scores = next_token_scores.to(next_token_logits.dtype)
            # reshape for beam search
            ...
            candidate_token_scores, candidate_tokens = torch.topk(
                next_token_scores, num_attn_candidates, dim=-1, largest=True, sorted=True
            ...
            attn_last = []
            crop_position = model_kwargs['past_key_values'][0][0].shape[-2]
            ...
            for candidate_id in range(num_attn_candidates):
                ...
                model_kwargs_tmp['past_key_values'].crop(crop_position)
                
                # prepare model inputs
                model_inputs_tmp = self.prepare_inputs_for_generation(input_ids_tmp, **model_kwargs_tmp)
                #print("model_inputs_tmp attention_mask shape: ",model_inputs_tmp["attention_mask"].shape)
            ...
            model_kwargs['past_key_values'].crop(crop_position)
            ...
            try:
                if all((rollback_loc_gather == rollback_loc).long().sum() > int(threshold) for _, rollback_loc_gather in enumerate(rollback_loc_gathers)):
                    ...
                    if "inputs_embeds" in history_states[-2].keys():
                        inputs_embeds = history_states[-2]["inputs_embeds"]
                    ...
                    if "images" in model_kwargs_ori.keys():
                        ...
                        #print("model_inputs_tmp['input_ids'] shape: ",model_inputs_tmp["input_ids"].shape)
                    else:
                        ...
                        #if len(model_kwargs["past_key_values"]) > 0:
                        #    model_kwargs["past_key_values"].crop(model_inputs_tmp["input_ids"].shape[-1]-1)
                        #print("model_inputs_tmp['input_ids'] shape: ",model_inputs_tmp["input_ids"].shape)
                    """
                    
                    cache_tmp = copy.deepcopy(model_kwargs["past_key_values"])
                    model_kwargs = copy.deepcopy(model_kwargs_ori)
                    
                    if "inputs_embeds" in history_states[-2].keys():
                        # we use the input_embeds of the previous state to extend the cache
                        model_kwargs["inputs_embeds"] = inputs_embeds
                        temp_len = input_ids.shape[1]+inputs_embeds.shape[1]
                        model_kwargs["past_key_values"] = cache_tmp
                        model_kwargs["past_key_values"].crop(temp_len-2)
                        model_kwargs["attention_mask"] = torch.cat([
                            model_kwargs["attention_mask"], torch.ones((
                                inputs_embeds.shape[0], temp_len-1 - model_kwargs["attention_mask"].shape[1]
                            )).to(inputs_embeds.device)], 1)
                        model_kwargs["cache_position"] = torch.tensor([temp_len-2]).to(inputs_embeds.device)
                        model_inputs_tmp = self.prepare_inputs_for_generation(input_ids[:, :-1], **model_kwargs)
                        #model_inputs_tmp["past_key_values"].crop(temp_len-1)
                        outputs_tmp = self(
                            **model_inputs_tmp,
                            return_dict=True,
                            output_attentions=False,
                        ...
                        # extend the cache_position to match the input_ids length
                        model_kwargs['cache_position'] = torch.arange(input_ids.shape[-1]-1).to(input_ids.device)
                        model_inputs_tmp = self.prepare_inputs_for_generation(input_ids[:,:-1], **model_kwargs)
                    ...
                    next_token_logits = outputs.logits[:, -1, :]
                    # crop the past_key_values to match the input_ids length
                    #model_kwargs["past_key_values"].crop(model_kwargs["past_key_values"][0][0].shape[-2]-1)
            ...
            if return_dict_in_generate:
                ...
                if output_attentions:
                    try:
                        ...
                        logger.warning("No decoder attentions available")
                    if self.config.is_encoder_decoder:
                        cross_attentions += (outputs.cross_attentions,)
                ...
                if output_hidden_states:
                    try:
                        ...
                        logger.warning("No decoder hidden states available")
    ...
    def opera_beam_search_bk(
        ...
        **model_kwargs,
        ...
        OPERA-optimized version with reduced memory footprint.
        """
        key_position = generation_config.key_position
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        ...
        batch_size = len(beam_scorer._beam_hyps)
        num_beams = beam_scorer.num_beams
        vocab_size = self.config.vocab_size  # Define vocab_size here for later use
        ...
        # init attention / hidden states / scores tuples - only allocate if needed
        scores = () if (return_dict_in_generate and output_scores) else None
        raw_logits = () if (return_dict_in_generate and output_logits) else None
        beam_indices = (
            tuple(() for _ in range(batch_beam_size)) if (return_dict_in_generate and output_scores) else None
        ...
        # initialise the history variables - use lists for dynamic sizing instead of pre-allocating tensors
        history_states = []
        history_rollback_locs = None
        beam_next_tokens = None
        beam_idx = None
        ...
        # Use a smaller data type for max_rollback_time and initialize only what's needed
        max_rollback_time = torch.zeros(window_size, dtype=torch.int16, device=input_ids.device)
        history_length = window_size
        reject_token_pos_gather = [[] for _ in range(window_size)]
        
        # Store original model_kwargs but don't create an unnecessary deep copy
        model_kwargs_ori = {k: v for k, v in model_kwargs.items()}
        ...
        while True:
            ...
            # Define current states - only store what's necessary and use references where possible
            current_state = {}
            current_state["input_ids"] = input_ids.clone()
            current_state["beam_scorer"] = copy.deepcopy(beam_scorer)
            if beam_indices is not None:
                current_state["beam_indices"] = beam_indices.copy()
            else:
                current_state["beam_indices"] = None
            current_state["cur_len"] = cur_len
            
            # if model is using input_embeds, then we need to also keep track of the input_embeds
            if "inputs_embeds" in model_kwargs:
                current_state["inputs_embeds"] = model_kwargs["inputs_embeds"].clone()
            ...
            # forward pass to get next token
            with torch.cuda.amp.autocast(dtype=torch.float16):  # Use mixed precision for forward pass
                outputs = self(
                    **model_inputs,
                    return_dict=True,
                    output_attentions=output_attentions,
            ...
            if "past_key_values" not in model_kwargs:
                ...
                if beam_idx is None:  # First step with past_key_values
                    ...
                    zeros_shape = list(attn_previous.shape)
                    zeros_shape[-1] = 1
                    attn_previous = torch.cat([
                        attn_previous, 
                        torch.zeros(zeros_shape, device=attn_previous.device, dtype=attn_previous.dtype)
                    ], -1)
                    
                    curr_attn = outputs.attentions[-1]  # No need to copy
                    if curr_attn.size(-1) != attn_previous.size(-1):
                        print('breaking!')
                        # More efficient padding operation
                        pad_size = attn_previous.size(-1) - curr_attn.size(-1)
                        curr_attn = F.pad(curr_attn, (0, pad_size), mode='constant', value=0)
                        print('Padding output attention to match previous attention')
                        break
                        
                    # Get only necessary indices from beam_idx to save memory
                    attn_previous = torch.cat([
                        attn_previous[beam_idx], 
                        curr_attn.max(1, keepdim=True).values
                    ], -2)
            
            # Optimize to avoid redundant operations - only keep what's needed
            attn_previous = attn_previous.max(1, keepdim=True).values # [batch_size * num_beams, 1, q, kv]
            
            # Move to CPU to save GPU memory, only if history will be kept
            if len(history_states) < history_length:
                current_state["attn_previous"] = attn_previous.detach().cpu()
            ...
            next_token_scores = F.log_softmax(
                next_token_logits, dim=-1, dtype=torch.float32
            )  # (batch_size * num_beams, vocab_size)

            next_token_scores_processed = logits_processor(input_ids, next_token_scores)
            next_token_scores = next_token_scores_processed + beam_scores[:, None].expand_as(next_token_scores)

            # Select candidates - reuse tensors where possible
            if num_attn_candidates < 1:
                raise ValueError(
                    f"Num of candidates must be larger than 0, but it is currently {num_attn_candidates}."
                )
            ...
            if len(history_states) < history_length:
                current_state["candidate_tokens"] = candidate_tokens.clone()
                current_state["beam_scores"] = beam_scores.clone()
                if beam_next_tokens is not None:
                    current_state["beam_next_tokens"] = beam_next_tokens.clone()
                else:
                    current_state["beam_next_tokens"] = None
                if beam_idx is not None:
                    current_state["beam_idx"] = beam_idx.clone()
                else:
                    current_state["beam_idx"] = None
            
            # Walk through all candidates to get their self-attention weights
            attn_last = []
            crop_position = model_kwargs['past_key_values'][0][0].shape[-2]
            
            # Process each candidate more efficiently
            for candidate_id in range(num_attn_candidates):
                # update temporary generated ids, model inputs, and length for next step
                input_ids_tmp = torch.cat([input_ids, candidate_tokens[:, candidate_id].unsqueeze(-1)], dim=-1)

                # Use shallow copy for model_kwargs_tmp
                model_kwargs_tmp = {k: v for k, v in model_kwargs.items() if k != 'past_key_values'}
                model_kwargs_tmp['past_key_values'] = model_kwargs['past_key_values']
                ...
                # Use mixed precision for forward pass to save memory
                with torch.cuda.amp.autocast(dtype=torch.float16):
                    outputs_tmp = self(
                        **model_inputs_tmp,
                        return_dict=True,
                        output_attentions=output_attentions,
                        output_hidden_states=False,  # Save memory by not generating hidden states
                    )
                
                attn_output = outputs_tmp.attentions[-1].max(1, keepdim=True).values 
                
                # Optimize tensor operations to reduce memory usage
                zeros_shape = list(attn_previous.shape)
                zeros_shape[-1] = 1
                ...
                del input_ids_tmp, model_kwargs_tmp, model_inputs_tmp, outputs_tmp, attn_output

            # Clean up the past key values
            model_kwargs['past_key_values'].crop(crop_position)

            # Gather the attentions of all candidates
            attn_last = torch.cat(attn_last, 1) # [batch_size * num_beams, num_attn_candidates, q+1, kv+1]
            
            # Use in-place operations for normalization to save memory
            attn_sum = attn_last.sum(-1, keepdim=True)
            attn_last = torch.div(attn_last, attn_sum)
            ...
            # Scale up the self-attention weights and calculate the scores
            # Use in-place operations where possible
            attn_local.mul_(scale_factor)
            
            # Optimize memory usage for attn_local_scores
            attn_local_scores = torch.zeros(
                (attn_local.shape[0], attn_local.shape[1], attn_local.shape[-1]), 
                dtype=torch.float32,  # Use float32 for better precision in calculations
                device=candidate_token_scores.device
            )
            
            for j in range(attn_local.shape[-1]):
                # Use more memory-efficient operations
                local_score = torch.prod(attn_local[..., j:, j], dim=-1).mul_(1e-7)
            ...
            # We use the rollback scores to penalize the subsequent tokens
            rollback_scores, rollback_locs = attn_local_scores.max(-1) # [batch_size * num_beams, num_attn_candidates]
            rollback_loc = rollback_locs.mode().values # [batch_size * num_beams]
            rollback_loc = rollback_loc.mode().values # [1]

            # Choose penalty approach based on response length
            penalty_scores = torch.where(
                torch.tensor(cur_response_lens <= 10, device=attn_scores.device),
                -attn_scores,
                rollback_scores
            )
            ...
            # Use in-place subtraction for candidate_token_scores
            candidate_token_scores.sub_(penalty_weights * penalty_scores)
            
            if len(history_states) < history_length:
                current_state["candidate_token_scores"] = candidate_token_scores.clone()

            # history check - maintain fixed size history
            if len(history_states) >= history_length:
                history_states.pop(0)
            history_states.append(current_state)

            # check if we need rollback - wrap in try/except for safety
            try:
                rollback_needed = all(
                ...
                if rollback_needed:
                    if rollback_loc < 10:
                        raise ValueError("Rollback position too small")
                    ...
                    if max_rollback_time[rollback_pos] >= num_attn_candidates:
                        rollback_pos = rollback_pos - 1
                        if max_rollback_time[rollback_pos] >= num_attn_candidates:
                            raise ValueError("Exceeded max rollback attempts")
                        else:
                            max_rollback_time[rollback_pos] += 1
                    else:
                        max_rollback_time[rollback_pos] += 1
                    ...
                    # discard the rollbacked states in history - more efficient loop
                    discard_count = cur_response_lens - rollback_pos - 2
                    ...
                    # Revive all variables in the state of the rollback position
                    if len(history_states) < 2:
                        raise ValueError("Not enough history states for rollback")
                        
                    input_ids = history_states[-2]["input_ids"]
                    beam_scorer = history_states[-2]["beam_scorer"]
                    if "inputs_embeds" in history_states[-2]:
                        inputs_embeds = history_states[-2]["inputs_embeds"]
                    beam_indices = history_states[-2]["beam_indices"]
                    cur_len = history_states[-2]["cur_len"]
                    
                    # Move tensors back to the appropriate device
                    attn_previous = history_states[-2]["attn_previous"].to(input_ids.device)
                    candidate_token_scores = history_states[-2]["candidate_token_scores"]
                    candidate_tokens = history_states[-2]["candidate_tokens"]
                    ...
                    # Efficiently handle model state reconstruction
                    cache_tmp = model_kwargs["past_key_values"]
                    # Use shallow copy for model_kwargs
                    model_kwargs = {k: v for k, v in model_kwargs_ori.items()}
                    
                    if "inputs_embeds" in history_states[-2]:
                        # Use input_embeds of previous state to extend cache
                        model_kwargs["inputs_embeds"] = inputs_embeds
                        temp_len = input_ids.shape[1] + inputs_embeds.shape[1]
                        model_kwargs["past_key_values"] = cache_tmp
                        model_kwargs["past_key_values"].crop(temp_len-2)
                        
                        # Create attention mask as needed
                        attention_expand = torch.ones(
                            (inputs_embeds.shape[0], temp_len-1 - model_kwargs["attention_mask"].shape[1]),
                        ...
                        model_kwargs["cache_position"] = torch.tensor([temp_len-2], device=inputs_embeds.device)
                        model_inputs_tmp = self.prepare_inputs_for_generation(input_ids[:, :-1], **model_kwargs)
                        
                        # Use mixed precision for processing
                        with torch.cuda.amp.autocast(dtype=torch.float16):
                            outputs_tmp = self(
                                **model_inputs_tmp,
                                return_dict=True,
                        ...
                        # Set cache position
                        model_kwargs['cache_position'] = torch.arange(input_ids.shape[-1]-1, device=input_ids.device)
                        model_inputs_tmp = self.prepare_inputs_for_generation(input_ids[:,:-1], **model_kwargs)
                    ...
                    # Use mixed precision for final forward pass
                    with torch.cuda.amp.autocast(dtype=torch.float16):
                        outputs = self(
                            **model_inputs_tmp,
                            return_dict=True,
                    ...
                    # set penalty on the corresponding candidates - optimize operations
                    min_logits = next_token_logits.min(-1, keepdim=True).values
                    next_token_logits.sub_(999. + min_logits)
                    next_token_logits = next_token_logits.view(batch_size, num_beams * vocab_size)
                    ...
                    if reject_token_pos_gather[rollback_pos]:
                        reject_token_pos = torch.cat([
                            torch.tensor(reject_token_pos_gather[rollback_pos], device=reject_token_pos.device), 
                            reject_token_pos
                        ], -1)
                        
                    reject_token_pos_gather[rollback_pos] = reject_token_pos.tolist()  # Store as list to save memory
                    
                    # Use scatter_ for in-place modification
                    next_token_logits.scatter_(-1, reject_token_pos, -999.)
                    next_token_logits = next_token_logits.view(batch_size * num_beams, vocab_size)
                    
                else:
                    raise ValueError("No rollback needed")
                    
            except Exception as e:
                # Fill and scatter in one operation when possible
                next_token_logits.fill_(-999.)
            ...
            # Compute next token scores with better memory efficiency
            next_token_scores = F.log_softmax(next_token_logits, dim=-1)
            next_token_scores_processed = logits_processor(input_ids, next_token_scores)
            next_token_scores = next_token_scores_processed + beam_scores[:, None].expand_as(next_token_scores)

            # Store scores, attentions and hidden_states when required - only if returning dict
            if return_dict_in_generate:
                if output_scores:
                    scores += (next_token_scores_processed,)
                if output_logits:
            ...
            # stateless beam scoring process
            beam_outputs = beam_scorer.process(
                input_ids,
                next_token_scores,
                next_tokens,
            ...
            # Update input_ids with next tokens - use in-place operations when possible
            input_ids = torch.cat([input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)

            # Update model_kwargs for next iteration
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
            ...
            # Update beam indices if we're tracking them
            if return_dict_in_generate and output_scores:
                beam_indices = tuple((beam_indices[beam_idx[i]] + (beam_idx[i],) for i in range(len(beam_indices))))
        ...
        # Build return output based on requested format
        if return_dict_in_generate:
            if not output_scores:
                sequence_outputs["sequence_scores"] = None
    ...
    def _vcd_sample(
        ...
        **model_kwargs,
        ...
        cross_attentions = () if (return_dict_in_generate and output_attentions) else None
        decoder_hidden_states = () if (return_dict_in_generate and output_hidden_states) else None
        cd_hidden_states = () if (return_dict_in_generate and output_hidden_states) else None
        ...
        #model_kwargs.update({"input_ids_cd": generation_config.vcd_inputs["input_ids"]}, None)
        model_kwargs_cd = deepcopy(model_kwargs)
        
        model_kwargs_cd.update({"images_cd": generation_config.vcd_inputs})

        #if generation_config.model_type == "janus":
        #    model_kwargs_cd.update({"inputs_embeds": generation_config.vcd_inputs})
        #elif generation_config.model_type == "emu3":
        #    input_ids_cd = generation_config.vcd_inputs
        ...
        while True:
            ...
            use_cd = hasattr(generation_config, "vcd_sample") and generation_config.vcd_sample
            use_sid = hasattr(generation_config, "use_sid") and generation_config.use_sid
            output_attentions_wo_img = True
            output_hidden_states_wo_img = True
            ...
            if use_cd: #for VCD and SID
                ...
                cd_alpha = 0.5#model_kwargs.get("cd_alpha") if model_kwargs.get("cd_alpha") is not None else 0.5
                cd_beta = 0.1#model_kwargs.get("cd_beta") if model_kwargs.get("cd_beta") is not None else 0.1
                ...
                cd_logits = diffs#.masked_fill(next_token_logits < cutoff, -1e6)

                ## cd_comments: apply temperature warping and top-k filtering in contrastive decoding
                cd_logits = cd_logits.to(input_ids.device)
                ...
                if sample_greedy: # argmax
                    ...
                    #cd_logits[cd_logits == float('-inf')] = -1e7
                    
                    cd_probs = nn.functional.softmax(cd_logits, dim=-1)
                    #cd_probs = torch.clamp(cd_probs, min=0.0, max=1.0)
                    #cd_probs = cd_probs / cd_probs.sum(dim=-1, keepdim=True)
                ...
                # Add position_ids to match causal mask size
                if 'position_ids' in model_inputs_cd:
                    position_ids = model_inputs_cd['position_ids']
                    attention_mask = model_inputs_cd.get('attention_mask')
                    if attention_mask is not None:
                        # Make position_ids match attention mask size
                        position_ids = position_ids[:, :attention_mask.size(-1)]
                        model_inputs_cd['position_ids'] = position_ids
                ...
                cd_logits = next_token_logits_cd
            ...
            if return_dict_in_generate:
                ...
                if output_hidden_states:
                    ...
                    cd_hidden_states += (
                        (outputs_cd.decoder_hidden_states,)
                        if self.config.is_encoder_decoder
                        else (outputs_cd.hidden_states,)
                    )
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                ...
                return SampleDecoderOnlyOutput(
                    ...
                    hidden_states=decoder_hidden_states,
                    cd_probs=cd_probs,
                    cd_hidden_states=cd_hidden_states,
                )
        else:
            return input_ids
